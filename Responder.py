...
from utils import *
import struct
os.environ["PATH"] = os.environ.get("PATH", "") + ":/sbin:/usr/sbin:/bin:/usr/bin"
banner()
...
parser.add_option('--noess', action="store_true", help="Disable ESS in MSCHAPv2 challenges. Default: False", dest="NOESS_On_Off", default=False)
parser.add_option('--dhcp',                action="store_true", help="Enable answers for DHCP broadcast requests. This option will inject a WPAD server in the DHCP response. Default: False", dest="DHCP_On_Off", default=False)
parser = optparse.OptionParser(usage='python %prog -I eth0 -w -r -f\nor:\npython %prog -I eth0 -wrf', version=settings.__version__, prog=sys.argv)
parser.add_option('-A','--analyze',        action="store_true", help="Analyze mode. This option allows you to see NBT-NS, BROWSER, LLMNR requests without responding.", dest="Analyze", default=False)
parser.add_option('-I','--interface',      action="store",      help="Network interface to use, you can use 'ALL' as a wildcard for all interfaces", dest="Interface", metavar="eth0", default=None)
parser.add_option('-i','--ip',             action="store",      help="Local IP to use (only for OSX)", dest="OURIP", metavar="10.0.0.21", default=None)
parser.add_option('-6','--externalip6',    action="store",      help="Poison all requests with another IPv6 address than Responder's one.", dest="ExternalIP6", metavar="2001:db8::1", default=None)
parser.add_option('-e','--externalip',     action="store",      help="Poison all requests with another IP address than Responder's one.", dest="ExternalIP", metavar="10.0.0.22", default=None)
parser.add_option('-b','--basic',          action="store_true", help="Return a Basic HTTP authentication. Default: NTLM", dest="Basic", default=False)
parser.add_option('-r','--wredir',         action="store_true", help="Enable answers for netbios wredir suffix queries. Answering to wredir will likely break stuff on the network. Default: False", dest="Wredirect", default=False)
parser.add_option('-d','--NBTNSdomain',    action="store_true", help="Enable answers for netbios domain suffix queries. Answering to domain suffixes will likely break stuff on the network. Default: False", dest="NBTNSDomain", default=False)
...
parser.add_option('--lm',                  action="store_true", help="Force LM hashing downgrade for Windows XP/2003 and earlier. Default: False", dest="LM_On_Off", default=False)
parser.add_option('--noess',               action="store_true", help="Disable ESS in MSCHAPv2 challenges. Default: False", dest="NOESS_On_Off", default=False)
parser.add_option('--dhcp',                action="store_true", help="Enable answers for DHCP broadcast requests. Default: False", dest="DHCP_On_Off", default=False)
parser.add_option('--dhcp-dns',            action="store",      help="DNS to inject in DHCP response. Default: None", dest="DHCP_DNS", default=None)
parser.add_option('--ttl',                 action="store",      help="Set the TTL for poisoned responses. Use 'RANDOM' for random TTL. Default: None", dest="TTL", default=None)
parser.add_option('--quiet',               action="store_true", help="Quiet mode, do not print NBT-NS, LLMNR, MDNS requests. Default: False", dest="Quiet", default=False)
parser.add_option('--answer-name',         action="store",      help="Force Responder to answer with a specific name. Default: None", dest="AnswerName", default=None)
parser.add_option('--error-code',          action="store",      help="Force Responder to answer with a specific error code. Default: None", dest="ErrorCode", default=None)
