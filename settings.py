...
class Settings:
	...
	def populate(self, options):
		...
		try:
			NetworkCard = subprocess.check_output(["/sbin/ifconfig", "-a"])
		except:
			try:
				NetworkCard = subprocess.check_output(["/sbin/ip", "address", "show"])
		...
		try:
			...
			try:
		    		# resolvectl not available (e.g. OpenWRT), read resolv.conf directly
			        ...
			        dns_content = ""
			        p = None
		RoutingInfo = b"Routing Info Unavailable"
		try:
			RoutingInfo = subprocess.check_output(["netstat", "-rn"])
		except:
			try:
				RoutingInfo = subprocess.check_output(["/sbin/ip", "route", "show"])
			except subprocess.CalledProcessError as ex:
				RoutingInfo = "Error fetching Routing information:", ex
				pass
		NetworkCard = b"Network Info Unavailable"
